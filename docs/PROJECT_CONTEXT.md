...
Ship Visual V3 art polish: `docs/SHIP_VISUAL_V3_PERFORMANCE_FIRST_PLAN.md` is APPROVED. Eleven new illustrated ship sprites have been prepared as an optimized local 1024×768 lossless WebP atlas (800,054 bytes / 781.3 KiB), with original crops and 64/78/128px contact-sheet previews. The raster binary is not committed to GitHub yet; `scripts/install-ship-v3.mjs` verifies its SHA-256 and registers it in a local repository clone. V3-aware rendering avoids duplicate thrusters and heavy shadow bloom. V2 remains active until the verified binary and manifest are committed. Paired real-browser V2/V3 frame pacing and two-build visual QA have not been conducted. M22–M25 remain pending.
