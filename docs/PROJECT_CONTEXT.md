...
The current production contract has three layers:

- `docs/WORLD_BACKGROUND_SCENE_SYSTEM_PLAN.md` — World mapping, scene identity,
  static cache and base scene architecture;
- `docs/CINEMATIC_BACKGROUND_SYSTEM_PLAN.md` — runtime motion, parallax,
  forward-flight sensation and cinematic environmental motion;
- `docs/LAYERED_BACKGROUND_ASSET_SYSTEM_PLAN.md` — authoritative visual
  direction for authored sky/planet/vortex/asteroid/landmark imagery and
  layered asset composition.

The production renderer must read as a living place in motion. Authored local
assets provide the primary scenic identity; procedural lines/particles are
support effects only.
...
Pre-M22 Recall Mode V1: IMPLEMENTATION COMPLETE on PR #110; CI #656 PASS · 149/149 test files · 748/748 tests · TypeScript + production build + audio/bundle/Ship-V3 budgets PASS. Recall reuses Campaign/World/stage/vocabulary/RPG systems, limits normal enemies to one at a time, hides English behind Recall Core slots, speaks each active enemy/layer/boss prompt through the existing speech ducking pipeline, disables hostile projectile/formation pressure, preserves enemy collision damage, adds Beginner→Extreme presets, optional Vietnamese/IPA, replay + Reveal Letter assists, and per-word learning-memory telemetry. Browser pronunciation/readability/boss/contact validation remains part of the M22 manual gate; M23 is still blocked.
Recall R07 adaptive learning follow-up: IMPLEMENTATION COMPLETE on PR #111; CI #665 PASS · 149/149 test files · 749/749 tests · TypeScript/build/audio/bundle/Ship-V3 guards PASS. Recall stage-start vocabulary now adds bounded extra weight only for memory-identified weak words while preserving every original source entry, capped at +25% / 512 refs. Stage Results expose Recall perfect/review, hint/replay and response-time metrics; mistake review sorts missed/corrected words first. No PlayerSave migration or frame-loop memory scan was added. Real-browser/audio M22 validation remains pending.
...
Pre-M22 polish slice: `docs/PRE_M22_UI_RECALL_CHARACTER_POLISH_PLAN.md` P00-P08.1 COMPLETE. RPG HUD slice `docs/PRE_M22_RPG_HUD_HOTBAR_PLAN.md` H01-H06 COMPLETE with PlayerSave v26. Stage-transition slice `docs/PRE_M22_STAGE_TRANSITION_PLAN.md` T01-T06 COMPLETE on PR #70 · CI #455 PASS · 121/121 test files · 613/613 tests · TypeScript/build/bundle PASS: every stage gets a short skippable pre-combat transition; World/Galaxy/Boss/Hidden encounters escalate visually; the transition resolves before `game.startStage()`, so combat cannot run behind it. M23 remains blocked until the final M22 manual gate passes. Combat Identity + Typing Clarity slice `docs/PRE_M22_COMBAT_IDENTITY_TYPING_CLARITY_PLAN.md` C01-C12 is implementation-complete on PR #71 with CI #488 PASS (124/124 files, 629/629 tests, TypeScript/build/bundle), pending merged-main CI and real-browser M22 manual QA: active enemy word selection softly suppresses same-prefix conflicts inside near-Rank bands, same-initial targeting is deterministic nearest-first, Test Lab includes a fixed same-prefix scenario, 11 illustrated player ships load through the existing fail-soft art pipeline, aura derives from current equipment without new persistence, and character-specific tracer/impact VFX reuse the existing Laser mechanic.
~~~

Ship Visual V3 art polish: the reviewed premium atlas is now committed and active by default. Commit `6756a943` added `public/assets/space-typing/ships/player-ships-v3.webp` (1024×768, 800,054 bytes / 781.3 KiB, reviewed SHA-256 `fb9434e002d6da650e34192eb425e62d1e2f3bec8804a9b33b7aa8733de10eb3`) and the production manifest registers `player-ship-sheet-v3`. Startup selects V3 → V2 → procedural; `?shipArt=v2` remains the clean A/B baseline and successful V3 selection releases the decoded V2 image reference. Production build runs the dedicated hash/header/dimension/file-size guard; CI #606 reports Ship V3 art budget PASS. **The remaining V3 blocker is not binary installation:** paired same-device real-browser V2/V3 performance, 64/78/128px readability/visual review and final M22 manual evidence are still pending.
