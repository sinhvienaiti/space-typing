...
- Ship Visual V3 when the reviewed binary is installed: read `document.documentElement.dataset.shipArt` in Test Lab; it must be `v3` for premium artwork and `v2` when absent. Compare the same Stage/quality/build in the same browser: load `?shipArt=v2` for the baseline (V3 will not even preload), then reload without that query parameter for V3. Confirm `document.documentElement.dataset.shipArt` shows the correct selected art in each run. Perform three 60-second runs per quality, using the existing Test Lab profiler to record average/p95 frame ms and any slow-frame spikes; reject the premium-art switch if p95 regresses by over 1 ms. At native 64/78/128px review all 11 ships, two opposite equipment auras and fast-shot readability; reject the upgrade on >1ms p95 regression, obscured words or failed art load. Browser/human rows remain PENDING until observed on real hardware;
