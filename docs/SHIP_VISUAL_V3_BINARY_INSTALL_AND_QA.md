...
- Raw file: 800,054 bytes (781.3 KiB), recorded as an informational measurement rather than a hard acceptance ceiling.
...
`scripts/install-ship-v3.mjs` verifies the reviewed SHA, copies the binary into `public/`, and inserts the V3 art manifest entry just after V2. `scripts/check-ship-art-integrity.mjs` runs under `pnpm build` and rejects unregistered, invalid-header, wrong-resolution and unreviewed-hash V3 files while reporting transfer size.
...
- the reviewed SHA-256 guard in `scripts/check-ship-art-integrity.mjs`;
- startup selection `V3 → V2 → procedural`, with `?shipArt=v2` explicitly excluding V3 from preload for A/B measurements.

`pnpm build` verifies the binary hash, RIFF/WebP header and exact 1024×768 dimensions. Transfer size is reported but does not fail the build solely for crossing a historical byte ceiling. CI #606 remains historical evidence of the earlier guard configuration.
...
- No new ship animation loop or per-frame image decode may be introduced. If runtime profiling regresses, simplify or optimize the responsible visual work and re-test.
